...
namespace StarfieldDumping
{
    public class ByteSearch
    {
        ...
        public int Size => this._Values.Count;
        ...
        public static bool Match(byte[] buffer, int offset, int count, Pattern pattern, out int result)
        {
            ...
            for (int i = 0, o = offset; i + pattern.Count <= remaining; i++, o++)
            {
                bool matched = true;
                for (int j = 0; matched == true && j < pattern.Count; j++)
                {
                    var value = buffer[o + j] & pattern[j].Mask;
        ...
        public class Pattern : IEnumerable<PatternEntry>
        {
            ...
            public PatternEntry this[int i] => this._Entries[i];
